...
-- -----------------------------------------------------------------------------
-- Validation: referential integrity & enum usage smoke test
-- -----------------------------------------------------------------------------
DO $$
DECLARE
  fk_count INTEGER;
BEGIN
  ...
  -- 17 FKs: order_lines.product_id is intentionally not FK’d (historical snapshot may outlive catalog rows).
  IF fk_count < 17 THEN
    RAISE EXCEPTION 'Schema validation failed: expected at least 17 foreign keys, got %', fk_count;
  END IF;

  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'product_type') THEN
    RAISE EXCEPTION 'Schema validation failed: product_type enum missing';
  END IF;

  RAISE NOTICE 'CARNALYSYS schema OK: % foreign keys, enums present.', fk_count;
END $$;
