...
import { isActiveDeliveryOrder } from '../../lib/deliveryUiStage.js'
...
import { DeliveryWorkflowScreen } from '../components/delivery/DeliveryWorkflowScreen.jsx'
...
export function AdminOrderDetailsPage() {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      try {
        ...
        if (!cancel) setDeliveryQueue(Array.isArray(list) ? list : [])

      } catch {

        if (!cancel) setDeliveryQueue([])
  ...
  }, [isDelivery, order?.deliveryStage, order?.id])
  ...
  const nextOrderId = useMemo(() => {
    ...
    const active = deliveryQueue.filter((o) => isActiveDeliveryOrder(o))

    const idx = active.findIndex((o) => o.id === orderId)

    if (idx >= 0 && idx < active.length - 1) return active[idx + 1].id

    const other = active.find((o) => o.id !== orderId)

    return other?.id ?? null

  }, [deliveryQueue, orderId])
